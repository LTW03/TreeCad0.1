...
if ($resultChapterID->num_rows > 0) {
    ...
    if ($resultCourseAndExamType->num_rows > 0) {
        ...
        $sqlRandomQuestions = "SELECT QID_B, Question_B 
                               FROM final_question_b 
        ...
        if ($resultRandomQuestions->num_rows > 0) {
            ...
            while ($rowQuestion = $resultRandomQuestions->fetch_assoc()) {
                $questionID = $rowQuestion['QID_B'];
                $question = $rowQuestion['Question_B'];
                //question
                echo "<div class='question_container'>";
                echo "<p> {$question} </p>";

                // Get answers option
                $sqlAnswers = "SELECT F_B_Answer_ID, Final_Answer_B, Correct_Answer 
                               FROM final_b_answer 
                               WHERE QID_B = '$questionID'
                               ORDER BY RAND()";
                $resultAnswers = $conn->query($sqlAnswers);

                if ($resultAnswers->num_rows > 0) {
                    while ($rowAnswer = $resultAnswers->fetch_assoc()) {
                        $answerID = $rowAnswer['F_B_Answer_ID'];
                        $answer = $rowAnswer['Final_Answer_B'];
